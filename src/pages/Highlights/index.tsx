...
import type { Startup } from "../DashboardPitchInvest/types";
...
const Highlights: React.FC = () => {
	document.title = "Highlights | Pitch Invest";

	// For now, using mock data. Later this will fetch from Supabase:
	// - 6 most recently sold: WHERE status = 'sold' ORDER BY sold_at DESC LIMIT 6
	// - 6 most expensive: ORDER BY valuation DESC LIMIT 6
	// - Other highlight categories
	...
	const mostRecentlySold = mockStartups.slice(0, 6).map((p) => ({
		...p,
		status: "sold",
	}));
	...
	return (
		<div className="page-content">
			<Container fluid>
				<Row className="mb-4">
					<Col>
						<h4 className="mb-1">Highlights / Showcase</h4>
						<p className="text-muted mb-0">
							Discover featured profiles, top investments, and successful projects
						</p>
					</Col>
				</Row>

				<HighlightSection
					title="6 Most Recently Sold"
					profiles={mostRecentlySold}
				/>
				<HighlightSection
